...
/** @Author: fangl @Description: mybatis拦截器——分页(配合重写的执行器和分页sql生成工厂使用) @Date: 17:09 2018/10/30 */
@Intercepts({
  /**
   * 注意这个singnature,是很关键的，它决定返回代理对象后进不进入intercept，只有这个地方设定对应type后才会进行拦截
   * method是拦截类type中的方法，args是这个method中参数 注意这三个对象都可以在Invocation这个类中获取到
   */
  @Signature(
    method = "query",
    type = Executor.class,
...
public class MybatisPage implements Interceptor {
  ...
  private static final ReflectorFactory DEFAULT_OBJECT_REFLECTOR_FACTORY =
   ...
   * e.注意只有Signature标签中拦截了这个四个类其中一个，才会在plugins方法执行后，进入到intercept中 @Date: 11:41 2018/11/23
   */
  @Override
  ...
  /**
   * 2@Author: fangl @Description:
   * 如果plugin返回的是代理对象，则会在执行到plugin方法后立刻转到这个方法中，并且你可以在这个方法中针对那一步的代理对象做相应的操作 @Date: 11:43 2018/11/23
   */
  @Override
  ...
  /** 3@Author: fangl @Description: 拦截StatementHandler后要改装分页sql的地方 @Date: 22:54 2018/11/19 */
  private Object handleStatementHandler(Invocation invocation)
    ...
    /**
     * 执行语句对象(MetaObject是Mybatis提供的一个的工具类，通过它包装一个对象后可以获取或设置该对象的原本不可访问的属性) 1) MetaObject
     * forObject(Object object,ObjectFactory objectFactory, ObjectWrapperFactory
     * objectWrapperFactory)
     *
     * <p>2) Object getValue(String name)
     *
     * <p>3) void setValue(String name, Object value)
     *
     * <p>方法1)用于包装对象；方法2)用于获取属性的值（支持OGNL的方法）；方法3)用于设置属性的值（支持OGNL的方法）；
     */
  ...
  /** @Author: fangl @Description: mybatis初始化配置参数方法 @Date: 11:41 2018/11/23 */
  @Override
