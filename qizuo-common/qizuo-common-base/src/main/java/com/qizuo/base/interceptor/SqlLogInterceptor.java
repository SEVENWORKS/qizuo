...
public class SqlLogInterceptor implements Interceptor {
  ...
  public Object intercept(Invocation invocation) throws Throwable {
    ...
    if (enableSqlLogInterceptor) {
      ...
      Boolean flag =
          (Boolean) ThreadLocalMap.get(GlobalConstant.SqlConfig.NotDisplaySqlAspect_DISPLAY_SQL);
      // 当执行时间超过时间的会报错，这个错误表明执行时间太长了
      if (time
          >= GlobalConstant.SqlConfig.SqlLogInterceptor_NoticeTime
              * GlobalConstant.Number.THOUSAND_INT) {
