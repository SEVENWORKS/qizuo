<template>
  <div>
    <!-- 内容区域 -->
    <form class="block-area" id="dataContainer"></form>
    ...
    <div id="sendObj" style="display: none;" class="block-area">
      ...
      <div class="row m-b-10 sendObj_left" style="display: none;">
        ...
        <select id="sendUserId" class="select" multiple> </select>
      </div>
      <!-- 某一类 -->
                ...
                $.post(
                  ...
                  function (data) {
                    backResult(data, function () {
                      ...
                      $(".drawer-toggle i")
                        .eq(1)
                        .text(parseInt($(".drawer-toggle i").eq(1).text()) - 1);
                    });
                  }
                );
              }
    ...
    backResult(data, function (bt) {
      ...
      if (isNotBlank(bt)) {
        for (var i = 0; i < bt.length; i++) {
          html +=
            '<option value="' + bt[i].baseId + '">' + bt[i].name + "</option>";
        }
      } else {
