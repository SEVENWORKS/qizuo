  ...
  var settings = {},
    ...
    data = {
      ...
      addNodesData: function (setting, parentNode, nodes) {
        ...
        if (parentNode[childKey].length > 0) {
          parentNode[childKey][
            parentNode[childKey].length - 1
          ].isLastNode = false;
          view.setNodeLineIcos(
            setting,
      ...
      getNodesByParamFuzzy: function (setting, nodes, key, value) {
        ...
        for (var i = 0, l = nodes.length; i < l; i++) {
          if (
            typeof nodes[i][key] == "string" &&
            nodes[i][key].toLowerCase().indexOf(value) > -1
          ) {
            result.push(nodes[i]);
          }
          result = result.concat(
            data.getNodesByParamFuzzy(setting, nodes[i][childKey], key, value)
          );
        }
        return result;
      },
    ...
    view = {
      ...
      makeNodeIcoStyle: function (setting, node) {
        ...
        if (!node.isAjaxing) {
          var icon =
            node.isParent && node.iconOpen && node.iconClose
              ? node.open
                ? node.iconOpen
                : node.iconClose
              : node.icon;
  ...
  var _consts = {
    ...
    _zTreeTools = function (setting, zTreeTools) {
      ...
      zTreeTools.getCheckedNodes = function (checked) {
        ...
        return data.getTreeCheckedNodes(
          setting,
      ...
      zTreeTools.getChangeCheckedNodes = function () {
        var childKey = setting.data.key.children;
        return data.getTreeChangeCheckedNodes(
          setting,
          data.getRoot(setting)[childKey]
        );
      };
    ...
    _view = {
      ...
      makeChkClass: function (setting, node) {
        ...
        var chkName =
          ...
          "_" +
          (node[checkedKey] ? c.TRUE : c.FALSE) +
          "_" +
          fullStyle;
        chkName =
      ...
      setSonNodeCheckBox: function (setting, node, value, srcNode) {
        ...
        if (node[childKey]) {
          for (
            var i = 0, l = node[childKey].length;
            i < l && node.chkDisabled !== true;
            i++
          ) {
            var sNode = node[childKey][i];
  ...
  var _consts = {
    ...
    _zTreeTools = function (setting, zTreeTools) {
      ...
      zTreeTools.moveNode = function (targetNode, node, moveType, isSilent) {
        ...
        if (
          tools.canAsync(setting, targetNode) &&
          moveType === consts.move.TYPE_INNER
        ) {
            ...
            if (tmpTarget && tmpTarget.attr("id") != targetSetting.treeId) {
              ...
              if (!canPrev && !canNext && !canInner) {
                ...
                if (
                  ...
                  dY =
                    tmpNextA == null ||
                    (tmpTargetNode.isParent && tmpTargetNode.open)
    ...
    _view = {
      ...
      moveNode: function (
        ...
        if (!nodeDom.get(0)) {
          nodeDom = view
            .appendNodes(setting, node.level, [node], null, false, true)
            .join("");
